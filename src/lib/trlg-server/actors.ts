...
export class ActorStorage<S extends StorageAdapter<false>> {
    ...
    private constructor(gameId: string, initialJson: ActorStorageJSON, genAdapter: (init: ActorStorageJSON) => S) {
        this.gameId = gameId

        this.db = genAdapter(initialJson)
    ...
    public static generate<S extends StorageAdapter<false>>(gameId: string, initialJson: ActorStorageJSON, genAdapter: (init: ActorStorageJSON) => S) {
        return new ActorStorage<S>(gameId, initialJson, genAdapter)
    }
...
export class AsyncActorStorage<A extends StorageAdapter<true>> {
    ...
    public static async generate<A extends StorageAdapter<true>>(gameId: string, initialJson: ActorStorageJSON, genAdapter: (init: ActorStorageJSON) => Promise<A>) {
        const adapter = await genAdapter(initialJson)
        await adapter.load()
        return new AsyncActorStorage<A>(gameId, adapter)
    }

    public static async assign<A extends StorageAdapter<true>>(gameId: string, adapter: A) {
        adapter.load()
        return new AsyncActorStorage<A>(gameId, adapter)
    }
